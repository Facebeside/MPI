...
int main(int argc, char* argv[])
{
	...
	long long  low_index = id * N / p; //第一个数下标
	long long  high_index = (id + 1) * N / p - 1;//最后一个数下标
	...
	prime = 3;
	do {
		//从当前素数的平方开始寻找  如果current prime^2大于该节点的数据左端点 减去low_value再减半即为第一个倍数的下标
		...
		if (!id) {
			while (marked[++index]);
			prime = 2 * index + 3; //数据减半 下标index与其值value对应的关系为：value = 2 * index + 3
		}
	...
	if (!id) {
		//因为我们是从3开始筛选的，没有从2 开始，但2是素数的偶数，因此素数个数+1
